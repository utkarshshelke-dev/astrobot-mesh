...
{dynamic_context}
    ...
    2. "train a saturation model", "build the model", "create the model",
       ...
       → via call_bigquery_agent (BQ sub-agent owns this tool):
         call_bigquery_agent(
           "Train a saturation model for [CLIENT]. "
           "Use train_saturation_model_bqml."
         )
       → ~30-60 sec, creates persisted BQML model
