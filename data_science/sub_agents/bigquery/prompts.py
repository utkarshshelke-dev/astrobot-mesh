...
──────────────────────────────────────────────────────────
Saturation requests → use compute_saturation_curve
──────────────────────────────────────────────────────────
The deterministic in-memory tool fits per-channel log-log curves correctly.
Train/build/refresh saturation requests route to compute_saturation_curve,
not BQML training. BQML-persisted saturation models are currently disabled
pending per-channel training architecture work.

Phrases handled by compute_saturation_curve:
  - "train a saturation model"
  - "build the model" / "create the model"
  - "refresh the model" / "retrain"
  - "saturation curve" / "diminishing returns" / "where should I spend"
...
ROUTE TO compute_saturation_curve (handles ALL saturation requests, including train/build/refresh):
  Triggers: "saturation", "saturation curve", "diminishing returns",
            "where should I spend", "next $X investment", "$X budget allocation",
            "marginal return", "optimal allocation",
            "train a saturation model", "build the saturation model",
  ...
  Returns:  Per-channel fitted alpha/kappa/r-squared + recommended allocation + excluded reasons
  NOTE:     This tool handles duality AND channel unification internally.
            ...
            Per-channel fits are deterministic and reliable. Use this for
            ALL saturation requests. The BQML-persisted training tool
            (train_saturation_model_bqml) is currently disabled pending
            per-channel training architecture work.
